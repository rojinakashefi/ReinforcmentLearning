...
class RLTask(AbstractRLTask):
    ...
    def interact_sarsa(self, n_episodes):
        reward_episodes = []
...
def plot_rewards(rewards, method='Sarsa', room = ""):
...
if __name__ == "__main__":
    ...
    rewards = task.interact_sarsa(10000)
    plot_rewards(rewards, method='Sarsa', room="EMPTY_big")
...
#         if not done:
#             row_next, col_next, mon_r_next, mon_c_next = self._get_positions(next_state)
#         else:
#             row_next, col_next = self.grid_size[0] - 1, self.grid_size[1] - 1
#             mon_r_next, mon_c_next = self.grid_size[0], self.grid_size[1]

#         td_target = reward + self.gamma * self.q_matrix[row_next, col_next, mon_r_next, mon_c_next, next_action]
#         td_error = td_target - self.q_matrix[row, col, mon_r, mon_c, action]
#         self.q_matrix[row, col, mon_r, mon_c, action] += self.alpha * td_error
...
#     task = RLTask(env, agent)
#     rewards = task.interact_sarsa(10000)
#     plot_rewards(rewards, method="Sarsa", room="MONSTER")
#     task.visualize_episode()